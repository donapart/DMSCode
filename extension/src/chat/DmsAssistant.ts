...
export class DmsAssistant {
    ...
    private async handleSearch(
        ...
        if (results.length === 0) {
            ...
            for (const result of results.slice(0, 5)) {
                stream.markdown(`- 📄 **${result.document.name}** (${Math.round(result.score * 100)}% Relevanz)\n`);
    ...
    private async handleCompare(
        ...
        stream.markdown('Diese Funktion wird noch implementiert. Bitte wählen Sie zwei Dokumente aus.\n');
    ...
    private async handleGeneral(
        ...
        stream.progress('Verarbeite Anfrage...');
        ...
        stream.button({ command: 'dms.scanDocument', title: '📷 Scannen' });
