...
export function DocumentList() {
  ...
  const downloadDocument = async (objectName: string) => {
    try {
      const response = await fetch(
        `/api/storage/objects/${encodeURIComponent(objectName)}/url`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const deleteDocument = async (objectName: string) => {
    ...
    try {
      const response = await fetch(
        `/api/storage/objects/${encodeURIComponent(objectName)}`,
        {
          method: "DELETE",
        },
      );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredDocuments.map((doc) => (
            ...
            >
              ...
              {/* Name */}
              <h3
                className="font-medium text-dms-dark truncate"
                title={doc.name}
              >
                {doc.name.split("/").pop()}
              </h3>
              ...
              <div className="flex items-center gap-2 mt-2 text-xs text-dms-secondary">
                ...
                <span>•</span>
                <span>
                  {format(new Date(doc.last_modified), "dd.MM.yy", {
                    locale: de,
                  })}
                </span>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="text-left px-4 py-3 text-sm font-medium text-dms-secondary">
                  Name
                </th>
                <th className="text-left px-4 py-3 text-sm font-medium text-dms-secondary hidden md:table-cell">
                  Typ
                </th>
                <th className="text-left px-4 py-3 text-sm font-medium text-dms-secondary hidden sm:table-cell">
                  Größe
                </th>
                <th className="text-left px-4 py-3 text-sm font-medium text-dms-secondary hidden lg:table-cell">
                  Datum
                </th>
                <th className="text-right px-4 py-3 text-sm font-medium text-dms-secondary">
                  Aktionen
                </th>
              </tr>
            </thead>
              ...
              {filteredDocuments.map((doc) => (
                <tr key={doc.etag} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-dms-secondary hidden lg:table-cell">
                    {format(new Date(doc.last_modified), "dd.MM.yyyy HH:mm", {
                      locale: de,
                    })}
                  </td>
                  <td className="px-4 py-3">
